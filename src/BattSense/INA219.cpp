 ...
 */
bool INA219::begin(TwoWire *theWire, float rShunt) {
  if (!i2c_dev) {
...
float INA219::getCurrent_mA() {
  ...
  int16_t value;
  value = getShuntVoltage_raw() * 0.01;

  return value / _rShunt;
...
void INA219::setCalibration_32V_2A() {
  ...
  ina219_currentDivider_mA = 6.6; // Current LSB = 100uA per bit (1000/100 = 10)
...
void INA219::setCalibration_16V_400mA() {
  ...
  // only supporting 16V at 400mA max.
    ...
    float VBUS_MAX = 10;
    float VSHUNT_MAX = 0.004; //              (Assumes Gain 1, 40mV)
    float RSHUNT = 0.002; //              (Resistor value in ohms)
  ...
  // 4. Choose an LSB between the min and max values
  //    (Preferrably a roundish number close to MinLSB)
    float CurrentLSB = 0.00015; //(50uA per bit)
  ...
  //
  float Max_Current_Before_Overflow = (Max_Current > MaxPossible_I) ? MaxPossible_I : Max_Current;
 
  // Max_Current_Before_Overflow = MaxPossible_I
  // Max_Current_Before_Overflow = 0.4
  //
  float Max_ShuntVoltage = Max_Current_Before_Overflow * RSHUNT;

  float Max_ShuntVoltage_Before_Overflow = (Max_ShuntVoltage >= VSHUNT_MAX) ? VSHUNT_MAX : Max_ShuntVoltage;
  ...
  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 6.67;    // Current LSB = 50uA per bit (1000/50 = 20)
